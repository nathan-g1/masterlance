    ...
    it('returns the current health status of all microservices', () => {
      return request({
        ...
        .then(res => {
          ...
          expect(res.dependencies.Account.dependencies).to.deep.equal({ AccountDB: { status: 'healthy' } })
          expect(res.dependencies.Customer.dependencies).to.deep.equal({ CustomerDB: { status: 'healthy' } })
          expect(res.dependencies.Transaction.dependencies).to.deep.equal({ TransactionDB: { status: 'healthy' } })
        })
    })
  })
